 ...
 * Opis: Pobiera listę wszystkich produktów z bazy danych i wyświetla je w formie tabeli.
 * Zawiera logikę sprawdzania dostępności na podstawie magazynu i daty[cite: 22].
 */
function PokazProdukty() {
    ...
    while($row = mysqli_fetch_array($result)) {
        ...
        // --- Zestaw warunków dostępności (Laboratorium 11, Zadanie 1) ---
        // 1. Sprawdzenie statusu ogólnego [cite: 17]
        if ($row['status_dostepnosci'] == 0) $czy_dostepny = false;
        // 2. Sprawdzenie czy towar jest w magazynie [cite: 17]
        if ($row['ilosc_magazyn'] <= 0) $czy_dostepny = false;
        // 3. Sprawdzenie czy oferta nie wygasła [cite: 17, 27]
...
/**
 * Funkcja: FormularzDodawaniaProduktu
 * Opis: Generuje formularz HTML pozwalający wprowadzić nowy produkt.
 * Pobiera dostępne kategorie, aby przypisać produkt do konkretnej marki/modelu[cite: 15, 19].
 */
 ...
 * Opis: Pobiera dane istniejącego produktu o danym ID i wypełnia nimi formularz edycji[cite: 21].
 */
function EdytujProdukt($id) {
    ...
    // Formatowanie daty wygaśnięcia z bazy pod standard inputa HTML5 (datetime-local) [cite: 27]
    ...
    <div class="edycja">
        ...
        <form method="post" action="admin.php?funkcja=produkty&id='.$id.'">
            <label>Tytuł:</label><input type="text" name="edit_prod_tytul" value="'.htmlspecialchars($row['tytul']).'" required />
