...
/**
 * Funkcja: ZarzadzajKategoriami
 * Opis: Wyświetla listę kategorii w strukturze drzewiastej (Zadanie 1 - Lab 10/11).
 * Wykorzystuje zagnieżdżone zapytania SQL, aby przypisać modele do odpowiednich marek.
 */
function ZarzadzajKategoriami() {
    ...
    /**
     * PĘTLA POZIOMU 1: Kategorie główne (Marki)
     * Pobiera elementy, których 'matka' wynosi 0 (brak rodzica).
     */
    $res_matki = mysqli_query($link, "SELECT * FROM kategorie WHERE matka = 0 LIMIT 50");
    while ($matka = mysqli_fetch_assoc($res_matki)) {
        // Wyświetlanie wiersza marki (wyróżniony kolor tła i wielkie litery)
        ...
        /**
         * PĘTLA POZIOMU 2: Podkategorie (Modele)
         * Pobiera elementy, których rodzicem (matką) jest ID aktualnie przetwarzanej marki.
         */
        $id_m = $matka['id'];
...
/**
 * Funkcja: FormularzDodawaniaKategorii
 * Opis: Generuje formularz HTML do dodawania nowych pozycji.
 * Pozwala wybrać, czy nowa pozycja ma być główną marką, czy modelem przypisanym do marki.
 */
function FormularzDodawaniaKategorii() {
    global $link;
    
    // Pobranie tylko kategorii głównych (marek), aby wypełnić listę wyboru "Rodzic"
    $res = mysqli_query($link, "SELECT id, nazwa FROM kategorie WHERE matka = 0");
    
    // Opcja domyślna: jeśli użytkownik wybierze 0, stworzy nową markę główną
